...
/*
	...
	- 고유성: 중복 될 수 X
    - Not null: null 값이 될 수 X (반ㄷ싀 유효한 데이터를 포함)
...
# 1) 제약 조건 삭제
# : 기본 키 제약 조건 삭제 시 not null에 대한 조건은 사라지지 않는다.
...
# 외래 키 사용예시: 회원(Members) - 주문(Orders)
...
create table `orders` (
    ...
    member_id int
    # 외래 키 지정 방식
    # foreign key (참조 컬럼, 해당 테이블의 컬럼) references 기본 테이블(기본컬럼)
