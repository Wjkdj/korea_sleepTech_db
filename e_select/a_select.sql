...
# : 정렬을 하지 않고 조회 시 데이터 입력 순서대로 출력alter
...
#		: is null, in not null 키워드를 통해 null 여부만 확인 가능alter
...
select * from `members`
where
	name like 'J___'; # 시작이 J이고 뒤는 3개의 임의의 문자를 허용
...
select * from `members`
where
	name like '_u%'; # 1개의 임의의 문자 + u + 상관 없음: 이름의 두 번째 글자가 u인 모든 회원 조회alter
